...
@section('js')
    <script>
        ...
        $(document).on('click', '.add_payment', function (e) {
            e.preventDefault();
            $("#add_payments").append('<div class="box-body" id="payment"><div class="col-md-6"><div class="form-group"><select class="selectpicker form-control" data-live-search="true" data-style="btn-dropdownSelectNew" name="payment_user_id[]"><option value="" selected="selected">Select a User</option><?php foreach($users as $id => $name){ ?><?php echo "<option value=".$id.">$name</option>" ?><?php } ?></select></div></div><div class="col-md-5"><div class="form-group"><input class="form-control" step="any" placeholder="Amount" name="payment_amount[]" type="number"></div></div><div class="col-md-1"><button type="button" class="btn btn-danger btn-circle pull-right" id="removePayment"><i class="fa fa-minus"></i></button></div></div></div>');
            $('.selectpicker').selectpicker();
        });

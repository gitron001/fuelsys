	...
	<div class="content">
		<div class="row">
            ...
            <div class="box-header">
              <div class="col-md-6"><h3 class="box-title">{{ trans('adminlte::adminlte.branches') }}</h3></div>
              <div class="col-md-6">

                <form class="form-inline text-center pull-right" method="GET" action="{{ URL::to('/admin/branches') }}">
                  @include('includes.search_filter')
                  <a href="{{ url('admin/branches/create') }}" data-toggle="tooltip" class="btn btn-success pull-right" style="margin-left: 0.5em;" title="{{ trans('adminlte::adminlte.branches_details.create_new') }}"><i class="fa fa-plus"></i> {{ trans('adminlte::adminlte.new') }}</a>
                  <a data-toggle="tooltip" class="btn btn-danger" id="delsel" title="{{ trans('adminlte::adminlte.branches_details.delete_all') }}"><i class="fa fa-trash"></i> {{ trans('adminlte::adminlte.delete') }}</a>
                </form>
            ...
            <div class="box-body table-div">
              <table id="example2 orders" class="table table-bordered table-hover text-center">
                ...
                <tr>
                    ...
                    <th class="sorting" data-sorting_type="asc" data-column_name="address">{{ trans('adminlte::adminlte.address') }} <span id="address_icon" class="removePrevIcon sortIcon"><span class="glyphicon glyphicon glyphicon glyphicon-sort"></span></span></th>
                    <th class="sorting" data-sorting_type="asc" data-column_name="city">{{ trans('adminlte::adminlte.city') }} <span id="city_icon" class="removePrevIcon sortIcon"><span class="glyphicon glyphicon glyphicon glyphicon-sort"></span></span></th>

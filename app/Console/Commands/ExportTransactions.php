...
class ExportTransactions extends Command
{
    ...
    public function handle()
    {
        ...
        if($company->print_transaction == 1 && !empty($tr_location)){
            ...
            'transactions.price', 'transactions.lit','transactions.kilometers', 'transactions.money','transactions.id','transactions.channel_id','transactions.sl_no','transactions.tr_no','transactions.product_id','transactions.lit','transactions.price','transactions.money','transactions.created_at','transactions.bonus_user_id','transactions.user_id','transactions.channel_id')
            ...
            if(!file_exists($tr_location."/export-transactions.txt")){
                ...
                fwrite($fp, 'ID_PERSHT;DATAORA;KODART;ARTIKULLI;SASIA;CMIMI;REZERVUAR;RFID;Kilometrat;NrKuponit' .PHP_EOL);

                foreach($transactions as $transaction){

                    fwrite($fp, $transaction['id'].';'.date("d.m.Y H:i:s",strtotime($transaction['created_at'])).';'.$transaction['product_id'].';'.$transaction['product'].';'.$transaction['lit'].';'.$transaction['price'].';0;'.$transaction['staff_rfid'].';'.$transaction['kilometers'].';'.$transaction['id'].PHP_EOL);
